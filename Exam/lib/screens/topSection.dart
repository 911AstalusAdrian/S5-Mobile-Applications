...
import '../helpers/connectionCheck.dart';
...
class _TopSectionState extends State<TopSection> {
  ...
  var logger = Logger();
  bool online = true;
  late Map<String, int> topDoctors = <String, int>{};
  ...
  getList() async {
    ...
    try{
      if(online){
        topDoctors = await ServerHelper.instance.getTopDoctors();
      } else{
        ...
        child: ListView(
          children: [
            ListView.builder(itemBuilder: ((context, index) {
              return ListTile(
                  title: Text("${topDoctors.keys.elementAt(index)} | Symptoms associated - ${topDoctors.values.elementAt(index)}")
              );
            }
            ),
              itemCount: 3,
